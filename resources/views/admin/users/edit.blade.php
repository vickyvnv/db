                                                ...
                                                <div class="card-body">
                                                    ...
                                                    <form method="POST" action="{{ route('users.update', $user->id) }}">
                                                        ...
                                                        <div class="mt-4">
                                                            ...
                                                            <select id="roleSelect" name="role_id" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" onchange="updateAssignedUsers()">
                                                                @foreach($roles as $role)
                                                                    <option value="{{ $role->id }}" {{ old('role_id', $user->userRoles->pluck('id')->first()) == $role->id ? 'selected' : '' }}>{{ $role->name }}</option>
                                                                @endforeach
                                                            </select>
                                                            @error('roles')
                                                                <p class="text-red-500 mt-1 text-sm">{{ $message }}</p>
                                                        ...
                                                        <div class="mt-4" id="assignedUserContainer" style="display: none;">
                                                            ...
                                                            <select id="assigned_user_id" name="assigned_user_id" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
    ...
    if (roleId == 5) {
        ...
        .then(data => {
            ...
            data.forEach(user => {
                ...
                if (user.id == {{ old('assigned_user_id', $user->assignedUser->pluck('id')->first()) }}) {
